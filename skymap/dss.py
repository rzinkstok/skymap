...
if __name__ == "__main__":
    ...
    db = SkyMapDatabase()
    res = db.query_one("""SELECT * FROM ngc_ngc2000 WHERE Name=(SELECT name FROM ngc_names WHERE object LIKE '%{}%')""".format("M 104"))
    rah = res['RAh']
    ram = res['RAm']
    ...
    imdata = image.data
    im_max = numpy.max(numpy.max(imdata))
    imdata = numpy.rint(255*imdata/im_max).astype(numpy.int8)
